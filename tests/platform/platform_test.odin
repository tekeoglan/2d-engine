...
default_display_settings_are_valid :: proc(t: ^testing.T) {
	...
	testing.expect(t, settings.version == pl.DISPLAY_SETTINGS_VERSION, "display defaults should use the current schema version")
	testing.expect(t, settings.window.width == 1280, "display defaults should use the documented width")
	testing.expect(t, settings.window.height == 720, "display defaults should use the documented height")
	...
	testing.expect(t, settings.volume.master == 1.0, "display defaults should set master volume to unity")
	testing.expect(t, settings.volume.music == 1.0, "display defaults should set music volume to unity")
	testing.expect(t, settings.volume.effects == 1.0, "display defaults should set effects volume to unity")

	config := pl.platform_config_default()
	testing.expect(t, config.title == "2D Engine", "platform defaults should use the engine title")
	testing.expect(t, config.window == settings.window, "platform and display defaults should agree on window settings")
	testing.expect(t, config.request_high_dpi, "platform defaults should request high-DPI back buffers")
	testing.expect(t, config.settings_path == "settings.json", "platform defaults should provide a settings path")
}

@(test)
platform_init_rejects_invalid_values :: proc(t: ^testing.T) {
	ctx: pl.Platform_Context
	err := pl.platform_init(&ctx, pl.Platform_Config{window = pl.Window_Settings{width = 0, height = 720}})
	testing.expect(t, err.kind == .Invalid_Argument, "platform initialization should reject invalid dimensions")
	testing.expect(t, !ctx.is_initialized, "invalid platform configuration must not initialize the context")
}
...
event_translation_reports_quit_and_window_changes :: proc(t: ^testing.T) {
	...
	testing.expect(t, translated.state.logical_size == pl.Window_Size{1024, 576}, "resize should update logical size")
	testing.expect(t, translated.state.drawable_size == pl.Window_Size{2560, 1440}, "resize should preserve drawable size")
	...
	testing.expect(t, translated.state.drawable_size == pl.Window_Size{2048, 1152}, "pixel resize should update drawable size")
	testing.expect(t, translated.state.is_high_dpi, "different logical and drawable sizes should remain high-DPI")
	...
	ok = pl.platform_translate_sdl_event(&ctx, &pixel_resize, &translated)
	testing.expect(t, ok && !translated.state.is_high_dpi, "matching logical and drawable sizes should clear high-DPI state")

	quit := sdl.Event{type = .QUIT}
...
bounded_event_storage_preserves_quit :: proc(t: ^testing.T) {
	...
	testing.expect(t, events[0].kind == .Quit, "bounded event storage should preserve a terminal Quit event")
}
...
logical_and_drawable_sizes_remain_distinct :: proc(t: ^testing.T) {
	ctx := pl.Platform_Context{
		...
		gl_context_handle = cast(rawptr)uintptr(1),
		is_initialized = true,
		window_state = pl.Window_State{
			logical_size = pl.Window_Size{1280, 720},
			drawable_size = pl.Window_Size{2560, 1440},
	...
	testing.expect(t, state.logical_size != state.drawable_size, "logical and drawable sizes must be separate values")
...
platform_opengl_context_reports_loader_state :: proc(t: ^testing.T) {
	ctx := pl.Platform_Context{
		...
		gl_context_handle = cast(rawptr)uintptr(1),
		is_initialized = true,
		gl_info = pl.OpenGL_Context_Info{
			major_version = 3,
			minor_version = 3,
	...
	info := pl.platform_opengl_context_info(&ctx)
	testing.expect(t, info.major_version >= 3, "OpenGL context should report its major version")
	testing.expect(t, info.is_core_profile, "OpenGL context should report a core profile")
	testing.expect(t, info.loader_is_ready, "OpenGL context should report loader readiness")
}

@(test)
platform_shutdown_releases_owned_resources :: proc(t: ^testing.T) {
	ctx := pl.Platform_Context{is_initialized = true}
	...
	testing.expect(t, ctx.gl_context_handle == nil, "platform deinit should clear the GL context handle")
}
